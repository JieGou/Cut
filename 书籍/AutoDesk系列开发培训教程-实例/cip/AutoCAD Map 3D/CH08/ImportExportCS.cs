...
namespace CH08
{
	...
	public sealed class SampleCommands
	{
		...
		[Autodesk.AutoCAD.Runtime.CommandMethodAttribute("RunImport")]
		...
		[Autodesk.AutoCAD.Runtime.CommandMethodAttribute("RunExport")]
		...
		public bool ImportDwg(string format, string fileName, bool isToLayer0, bool isImpAttToOD)
		{
			...
			try
			{
				...
				if (null == m_ImpEventHandler)
				{
					...
					importer.RecordReadyForImport += new RecordReadyForImportEventHandler(m_ImpEventHandler.RecordReadyForImport);
					importer.RecordImported += new RecordImportedEventHandler(m_ImpEventHandler.RecordImported);
					importer.ImportRecordError += new ImportRecordErrorEventHandler(m_ImpEventHandler.RecordError);
				}
