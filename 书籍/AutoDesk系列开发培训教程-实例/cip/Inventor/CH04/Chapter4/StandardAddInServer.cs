...
namespace Chapter4
{
    ...
    /// </summary>
    [GuidAttribute("58171d3b-abfc-483d-a4c1-2254f8925c15")]
    public class StandardAddInServer : Inventor.ApplicationAddInServer
    {
        ...
        public void CreateWorkFeature(Inventor.Application inventorApplication)
        {
            PartDocument partDoc;
            partDoc = (PartDocument)inventorApplication.ActiveDocument;

            PartComponentDefinition partCompDef;
            partCompDef = partDoc.ComponentDefinition;

            TransientGeometry trans;
            trans = inventorApplication.TransientGeometry;

            Point pnt;
            pnt = trans.CreatePoint(2, 3, 4);

            WorkPoint workPoint1;
            workPoint1 = partCompDef.WorkPoints.AddFixed(pnt, false);

            WorkPoint workPoint2;
            pnt = trans.CreatePoint(0, 0, 0);
            workPoint2 = partCompDef.WorkPoints.AddFixed(pnt, false);

            WorkAxis workAxis;
            workAxis = partCompDef.WorkAxes.AddByTwoPoints(workPoint1, workPoint2, false);

            WorkPlane workPlane;
            workPlane = partCompDef.WorkPlanes.AddByLinePlaneAndAngle(workAxis, partCompDef.WorkPlanes["YZ Plane"], 45, false);

            workPlane.Name = "MyFirstWorkPlane";
        }
